...
async
Adding the async attribute to your <script> tag will download the file without blocking the HTML parser, but pause the HTML parser to execute the script once it has finished download.

defer
Similarly to async, adding the defer attribute to your <script> tag will download the file without blocking the HTML parser. However, once the script has been downloaded it will not execute immediately but wait until the HTML parser has completed.

Note that deferred scripts are executed in the order in which the appear in the DOM.
